 ...
 * sends through, and the OTP buffer `observeLoginCodes` registers for `/api/test/last-otp` to read
 * back. Building two fakes lets those two roles land on *different* objects — one wins the
 * transport slot, the other wins the observation slot — and the read-back then polls a buffer
 * nothing ever writes to. The signup form fires two auth calls in parallel, so this is the real
 * request shape, not a hypothetical.
 *
