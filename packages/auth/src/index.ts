...
// `peekLoginCode` and `peekMagicLink` are dev/test-only read-backs of a DI-injected fake's
// captured OTPs and sign-in links — inert in production (nothing registers a fake there). The
// in-memory fake itself lives on the test-only `@resonance/auth/testing` subpath (ADR-0018), NOT
// on this runtime entrypoint.
export {
  createResendMail,
  resolveMail,
  peekLoginCode,
